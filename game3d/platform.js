/* ---------------- the host door ----------------
   Ported from src/platform_base.js + src/platform_crazy.js. Every ⚠ below is a
   bug that was paid for once in the 2D build; none of them is theoretical.
   ...
   ⚠ This is the ONE file allowed to name a host. The platform forbids
   third-party ad networks outright, so a single line of another store's SDK in
   the uploaded build is a compliance failure.                                */
...
/** Budget for fetching the script alone, inside INIT_TIMEOUT_MS. Needed ON TOP
    ...
    either handler. Measured from Vietnam at 7.9s cold and 1.0-1.4s warm, so a
    cold player genuinely does lose the SDK here. */
const LOAD_TIMEOUT_MS = 2000;

/** ⚠ Fetched at runtime, never a tag in the page head: a blocking <script src>
    stops the HTML parser, and a slow answer from this host then stops
    everything after it. The platform hosts the file and forbids bundling it, so
    unlike three.js this one cannot be inlined - it has to stay a fetch. */
...
const PLATFORM = {
  ...
  async init() {
    ...
    try {
      ...
      //
      // ⚠ `started` decides whether we adopt it, NOT the race finishing. The
      // race finishes either way, and adopting a half-initialised SDK is worse
      // than having none: every later call reaches a live object that throws
      // "CrazySDK is not initialized yet", so the session logs a stream of
      // errors AND loses the local fallback it would otherwise have had. The
      // 2D build has this bug; it only shows up where init() is slow or cannot
      // complete at all, which is exactly a cold mobile connection.
      let started = false;
      await Promise.race([
        found.init().then(() => { started = true; }),
        new Promise(r => setTimeout(r, Math.max(0, INIT_TIMEOUT_MS - (Date.now() - start)))),
      ]);
      if (!started) return;              // no SDK is a cleaner answer than a broken one
      sdk = found;
